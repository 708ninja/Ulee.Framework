...
using OxLib.Utils;
using OxLib.Threading;

//------------------------------------------------------------------------------
namespace OxLib.Device.Plc
{
	public enum EOxProtocol { Tcp, Udp }

	//--------------------------------------------------------------------------
	public class OxMelsecQException : ApplicationException
	{
		public OxMelsecQException()
			: base("Occurred Melsec-Q PLC Exception!")
		{
		}

		public OxMelsecQException(string aMsg)
			: base(aMsg)
		{
		}
	}
	...
	//--------------------------------------------------------------------------
	public class OxMelsecQSerialNoException : OxMelsecQException
	{
		public OxMelsecQSerialNoException()
			: base("Occurred invalid serial no Exception!")
		{
		}

		public OxMelsecQSerialNoException(string aMsg)
			: base(aMsg)
		{
		}
	}
	...
	//--------------------------------------------------------------------------
	public class OxMelsecQTerminalCodeException : OxMelsecQException
	{
		public OxMelsecQTerminalCodeException()
			: base("Occurred terminal code Exception!")
		{
		}

		public OxMelsecQTerminalCodeException(string aMsg)
			: base(aMsg)
		{
		}
	}
	...
	//--------------------------------------------------------------------------
	public class OxMelsecQTimeoutException : OxMelsecQException
	{
		public OxMelsecQTimeoutException()
			: base("Occurred Melsec-Q PLC communication time out Exception!")
		{
		}

		public OxMelsecQTimeoutException(string aMsg)
			: base(aMsg)
		{
		}
	}
	...
	//--------------------------------------------------------------------------
	public abstract class OxQnA4EPacket
	{
		protected OxBinSets packet;
		...
		public OxQnA4EPacket(int aCount)
		{
			packet = new OxBinSets(aCount);
		}
	}

	//--------------------------------------------------------------------------
	public class OxQnA4ESendPacket : OxQnA4EPacket
	{
		// Cpu 감시 타이머
		...
		public OxQnA4ESendPacket(int aCount)
			: base(aCount)
		{
			Clear();
		}
	...
	//--------------------------------------------------------------------------
	public class OxQnA4ERecvPacket : OxQnA4EPacket
	{
		// 종료 코드
		...
		//----------------------------------------------------------------------
		public OxQnA4ERecvPacket(int aCount)
			: base(aCount)
		{
			Clear();
		}
		...
		public bool Bit(int aIndex)
		{
			...
			if ((aIndex % 2) == 0)
			{
				byValue = OxBinTrim.HiNibble(byValue);
			}
			else
			{
				byValue = OxBinTrim.LoNibble(byValue);
			}
	...
	public class OxMelsecQClient
	{
		...
		{ Connect, Disconnect, Send, Recieve, Comment, Exception }

		private string str;

		private bool connected;
		// 송수신 Protocol - Tcp, Udp
		private EOxProtocol protocol;
		// 송수신 IP
		private string ip;
		// 송수신 Port
		private int port;
		...
		private OxQnA4ESendPacket sPacket;
		// Melsec-Q PLC QnA 호환 4E Frame 수신Packet
		private OxQnA4ERecvPacket rPacket;
		...
		public EOxLogFileSeperation LogFSeperation
		...
		//----------------------------------------------------------------------
		public OxMelsecQClient(
			string aIP, 
			int aPort,
			EOxProtocol aProtocol=EOxProtocol.Tcp, 
			...
			int aPacketSize=csPacketSize)
		{
			...
			// 송수신 Packet 생성
			sPacket = new OxQnA4ESendPacket(packetSize);
			rPacket = new OxQnA4ERecvPacket(packetSize);
		}
		...
		public void Open()
		{
            ...
            if (protocol == EOxProtocol.Tcp)
            {
                ...
                {
                    str = "Occurred All TCP connection fail exception in OxMelsecQ.Open";
		...
		private void Send(int aCount)
		{
			switch (protocol)
			{
				case EOxProtocol.Tcp:
					SendTcp(aCount);
					break;

				case EOxProtocol.Udp:
					SendUdp(aCount);
					break;
			}
		}
		...
		private void SendTcp(int aCount)
		{
			for (int i = 0; i < tcpList.Count; i++)
			{
				for (int j = 0; j < csRetryCount; j++)
				{
					...
					catch (OxMelsecQTimeoutException)
					{
					}
					catch (OxMelsecQException)
					{
						break;
					}
					finally
				...
				{
					str = string.Format(
						"Changed TCP Port {0} -> {1} in OxMelsecQ.SendTcp",
						port + index - 1, port + index);
					LogEvent(EMQLogID.Comment, str);
					LogEvent(EMQLogID.Comment, "Retry sending packet in OxMelsecQ.SendTcp");
				}
			}

			LogEvent(EMQLogID.Exception, 
				"Occurred TCP communication error in OxMelsecQ.SendTcp");

			throw new OxMelsecQException(str);
		}
		...
		private void SendTcpPacket(int aCount)
		{
			...
			{
				str = "Occurred TCP sending error in OxMelsecQ.SendTcpPacket";
				LogEvent(EMQLogID.Exception, str);
		...
		private void SendUdp(int aCount)
		{
			...
			LogEvent(EMQLogID.Exception,
				"Occurred UDP communication error in OxMelsecQ.SendUdp");
			throw new OxMelsecQException(str);
		}
        ...
        private void SendUdpPacket(int aCount)
        {
            ...
            {
                str = "Occurred UDP sending error in OxMelsecQ.SendUdpPacket";
				LogEvent(EMQLogID.Exception, str);
		...
		private void RecieveTcp()
		{
			...
			{
				LogEvent(EMQLogID.Exception, 
					"Occurred TCP stream creation error in OxMelsecQ.RecieveTcp");
				throw new OxMelsecQException();
			}
			...
			try
			{
				...
				while (netStream.DataAvailable == false)
				{
					...
					{
						LogEvent(EMQLogID.Exception,
							"Occurred TCP recieve time out error in OxMelsecQ.RecieveTcp");
						throw new OxMelsecQTimeoutException();
					}
				...
				{
					LogEvent(EMQLogID.Exception,
						"Occurred TCP stream reading error in OxMelsecQ.RecieveTcp");
					throw new OxMelsecQException();
				}
		...
		private void RecieveUdp()
		{
			...
			try
			{
				...
				while (udp.Available == 0)
				{
					...
					{
						LogEvent(EMQLogID.Exception,
							"Occurred UDP recieve time out error in OxMelsecQ.RecieveUdp");
						throw new OxMelsecQTimeoutException();
					}
				...
				{
					LogEvent(EMQLogID.Exception,
						"Occurred UDP reading error in OxMelsecQ.RecieveUdp");
					throw new OxMelsecQException();
				}
		...
		private void Validate()
		{
			...
			{
				LogEvent(EMQLogID.Exception, 
					"Occurred mismatch serial no error in OxMelsecQ.Validate");
	
				throw new OxMelsecQSerialNoException();
			}
			...
			{
				str = string.Format(
					"Occurred abnormal terminal code #{0:X04} in OxMelsecQ.Validate",
					rPacket.Return);
				LogEvent(EMQLogID.Exception, str);

				throw new OxMelsecQTerminalCodeException(str);
			}
		}
		...
		public void GetBit(EMQDevice aDevice, UInt32 aAddr, UInt16 aCount=1)
		{
            if (aCount < 1)
			{
				str = "Occurred 'aCount' is low error in OxMelsecQ.GetBit";
				LogEvent(EMQLogID.Exception, str);
				throw new OxMelsecQException(str);
			}
		...
		public void GetWord(EMQDevice aDevice, UInt32 aAddr, UInt16 aCount=1)
		{
			if (aCount < 1)
			{
				str = "Occurred 'aCount' is low error in OxMelsecQ.GetWord";
				LogEvent(EMQLogID.Exception, str);
				throw new OxMelsecQException(str);
			}
	...
	//--------------------------------------------------------------------------
	public class OxMelsecQScaner
	{
		// 이전 PLC Device Data 값
		protected OxBinSets oldSets;
		// 현재 PLC Device Data 값
		protected OxBinSets newSets;
		// Melsec-Q PLC Ethernet Client
		private OxMelsecQClient client;
		// 주기적으로 PLC Data 읽는 Thread
		protected OxMelsecQScanThread thread;

		public OxMelsecQClient Client
		{ get { return client; } }

		public OxMelsecQScanThread.MelsecQScanHandler ScanningEvent
		{ set { thread.ScanningEvent += value; } }

		public OxMelsecQScanThread.MelsecQScanHandler AfterScanEvent
		{ set { thread.AfterScanEvent += value; } }

		//----------------------------------------------------------------------
        public OxMelsecQScaner(
			OxMelsecQClient aClient, int aCount, long aCycleTime)
        {
			client = aClient;

			// PLC Device 접점 저장소 할당
			oldSets = new OxBinSets(aCount);
			newSets = new OxBinSets(aCount);

			// 주기적으로 PLC Data 읽는 Thread 생성
			thread = new OxMelsecQScanThread(this, aCycleTime);
			thread.Resume();
		}
		...
		public void Byte(byte[] aValue)
		{
			...
			{
				throw new OxMelsecQException(
					"Occurred array length mismatch error in OxMelsecQDevice.Copy");
			}
    ...
    //--------------------------------------------------------------------------
    public sealed class OxMelsecQScanThread : OxThread
    {
		private long cycleTime;
		private OxMelsecQScaner scaner;

		public delegate void MelsecQScanHandler(OxMelsecQScaner aScaner);

		public event MelsecQScanHandler ScanningEvent;
		public event MelsecQScanHandler AfterScanEvent;

		//----------------------------------------------------------------------
		public OxMelsecQScanThread(OxMelsecQScaner aScaner, long aCycleTime)
        {
            scaner = aScaner;
        ...
        protected override void Execute()
        {
            ...
            while (Terminated == false)
            {
                ...
                {
					// Timer 초기화
                    nBeginTime = ElapsedMilliseconds;
